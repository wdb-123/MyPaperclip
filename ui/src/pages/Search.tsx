  ...
  if (showInitialState) {
    return (
      <div className="mx-auto flex w-full max-w-2xl flex-col gap-4 px-4 py-10 sm:px-6">
        <div>
          <h2 className="text-lg font-semibold">输入内容，搜索公司记忆。</h2>
          <p className="mt-1 text-sm text-muted-foreground">
            任务、评论、计划文档、代理和项目都会在同一个面板中按相关度排序。
          </p>
        </div>
        ...
        <ul className="space-y-1 text-xs text-muted-foreground">
          <li>
            <span className="font-medium text-foreground">编号直达：</span>输入{" "}
            <code className="rounded bg-muted px-1 py-0.5 text-[11px]">PAP-123</code> 可直接跳到对应任务。
          </li>
          <li>
            <span className="font-medium text-foreground">精确短语：</span>用引号包住短语，可匹配完整连续文本。
          </li>
          <li>
            <span className="font-medium text-foreground">⌘K：</span>重新打开命令面板，并带入当前搜索词。
          </li>
        </ul>
      </div>
    );
  ...
  if (hasError) {
    ...
    return (
      <div className="mx-auto flex w-full max-w-xl flex-col items-center justify-center gap-3 px-4 py-12 text-center">
        ...
        <div className="text-base font-semibold">无法执行这次搜索</div>
        <p className="text-sm text-muted-foreground">
          {status ? `服务器返回了 ${status}。` : "请求失败。"}你的输入和筛选仍会保留，可以重试，或改用任务筛选视图。
        </p>
  ...
  if (isEmpty) {
    return (
      <div className="mx-auto flex w-full max-w-xl flex-col items-center justify-center gap-3 px-4 py-12 text-center">
        ...
        <div className="text-base font-semibold">没有找到 &ldquo;{trimmedQuery}&rdquo; 的结果</div>
        <p className="text-sm text-muted-foreground">
          在{describeScope(scope)}中没有找到匹配项。可以扩大搜索范围，或换一种关键词。
        </p>
        <div className="flex flex-wrap items-center justify-center gap-2">
          {scope !== "all" ? (
        ...
        <ul className="mt-2 space-y-0.5 text-xs text-muted-foreground">
          <li>尝试减少词数，或只输入一个更有辨识度的关键词。</li>
          <li>
            也可以使用 <code className="rounded bg-muted px-1 py-0.5">PAP-123</code> 这样的编号直达。
          </li>
          <li>多词短语可用引号包住进行精确匹配。</li>
        </ul>
      </div>
    );
  }
  ...
  return (
    <div className="flex w-full max-w-[960px] flex-col px-2 sm:px-4" data-testid="search-results">
      <div className="flex items-center justify-between py-2 text-[11px] uppercase tracking-wide text-muted-foreground">
        <span>
          {totalResults} 个结果 · 按相关度排序
        </span>
        {isFetching ? <span aria-live="polite" className="normal-case tracking-normal">正在更新...</span> : null}
      </div>
