          ...
          <div className="min-w-0 space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              <span className="inline-flex items-center rounded-full border border-border bg-background px-2.5 py-1 text-[11px] font-medium uppercase tracking-[0.14em] text-muted-foreground">
                {summary.kind === "execution_workspace" ? t("projectWorkspace.kind.execution") : t("projectWorkspace.kind.project")}
              </span>
              <span className="inline-flex items-center rounded-full border border-border/70 bg-background px-2.5 py-1 text-xs text-muted-foreground">
                {t("projectWorkspace.updated", { time: timeAgo(summary.lastUpdatedAt, language) })}
              </span>
              {summary.serviceCount > 0 ? (
                ...
                >
                  ...
                  />
                  {t("projectWorkspace.services", { running: summary.runningServiceCount, total: summary.serviceCount })}
                </span>
              ) : null}
              {summary.executionWorkspaceStatus ? (
                <span className="inline-flex items-center rounded-full border border-border/70 bg-background px-2.5 py-1 text-xs text-muted-foreground">
                  {t(`projectWorkspace.status.${summary.executionWorkspaceStatus}`) === `projectWorkspace.status.${summary.executionWorkspaceStatus}`
                    ? summary.executionWorkspaceStatus.replace(/_/g, " ")
                    : t(`projectWorkspace.status.${summary.executionWorkspaceStatus}`)}
                </span>
              ) : null}
            </div>
            <Link
          ...
          >
            {summary.hasRuntimeConfig ? (
              ...
              >
                ...
                {hasRunningServices ? t("projectWorkspace.stopServices") : t("projectWorkspace.startServices")}
              </Button>
            ) : null}
            {summary.kind === "execution_workspace" && summary.executionWorkspaceId && summary.executionWorkspaceStatus ? (
              ...
              >
                {summary.executionWorkspaceStatus === "cleanup_failed" ? t("projectWorkspace.retryClose") : t("projectWorkspace.close")}
              </Button>
            ) : null}
          </div>
        </div>
          ...
          <div className="space-y-2 text-sm">
            {summary.branchName ? (
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{t("projectWorkspace.branch")}</div>
                  <div className="flex items-start gap-2">
                    <CopyText
                      text={summary.branchName}
                      containerClassName="min-w-0"
                      className="min-w-0 break-all text-left font-mono text-xs text-foreground"
                      copiedLabel={t("projectWorkspace.branchCopied")}
                    >
                      {summary.branchName}
                    </CopyText>
                    <CopyText
                      text={summary.branchName}
                      ariaLabel={t("projectWorkspace.copyBranch")}
            ...
            {summary.cwd ? (
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{t("projectWorkspace.path")}</div>
                  <div className="flex items-start gap-2">
                    <CopyText
                      text={summary.cwd}
                      title={summary.cwd}
                      containerClassName="min-w-0"
                      className="min-w-0 break-all text-left font-mono text-xs text-foreground"
                      copiedLabel={t("projectWorkspace.pathCopied")}
                    >
                      {truncatePath(summary.cwd)}
                    </CopyText>
                    <CopyText
                      text={summary.cwd}
                      ariaLabel={t("projectWorkspace.copyPath")}
                      className="mt-0.5 shrink-0 text-muted-foreground hover:text-foreground"
                      copiedLabel={t("projectWorkspace.pathCopied")}
                    >
                      <Copy className="h-3.5 w-3.5" />
            ...
            {summary.primaryServiceUrl ? (
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0">
                  <div className="text-[11px] uppercase tracking-[0.14em] text-muted-foreground">{t("projectWorkspace.service")}</div>
                  <a
        ...
        {summary.issues.length > 0 ? (
          <div className="space-y-2">
            <div className="text-[11px] font-medium uppercase tracking-[0.14em] text-muted-foreground">
              {t("projectWorkspace.linkedIssues")}
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              {hiddenIssueCount > 0 ? (
                ...
                >
                  {t("projectWorkspace.moreIssues", { count: hiddenIssueCount })}
                </Link>
              ) : null}
            </div>
          </div>
