...
export function AgentConfigForm(props: AgentConfigFormProps) {
  ...
  const createSecret = useMutation({
    mutationFn: (input: { name: string; value: string }) => {
      if (!selectedCompanyId) throw new Error(t("agent.error.selectCompanyCreateSecrets"));
  ...
  const {
    ...
    queryFn: () => {
      if (!selectedCompanyId) {
        throw new Error(t("agent.error.selectCompanyDetectModel"));
      }
  ...
  const testEnvironment = useMutation({
    mutationFn: async () => {
      if (!selectedCompanyId) {
        throw new Error(t("agent.error.selectCompanyTestEnvironment"));
      }
    ...
    props.onTestFeedbackChange({
      errorMessage: testEnvironment.error instanceof Error
        ...
        : testEnvironment.error
          ? t("agent.config.environmentTestFailed")
          : null,
  ...
  async function handleRefreshModels() {
    ...
    try {
      ...
      setRefreshModelsError(error instanceof Error ? error.message : t("agent.model.refreshFailed"));
    } finally {
    ...
    <div className={cn("relative", cards && "space-y-6")}>
      ...
      {isDirty && !props.hideInlineSave && (
        <div className="sticky top-0 z-10 flex items-center justify-end px-4 py-2 bg-background/90 backdrop-blur-sm border-b border-primary/20">
          <div className="flex items-center gap-3">
            <span className="text-xs text-muted-foreground">{t("agent.config.unsavedChanges")}</span>
            <Button
              size="sm"
              onClick={handleSave}
              disabled={!isCreate && props.isSaving}
            >
              {!isCreate && props.isSaving ? t("agent.config.saving") : t("agent.config.save")}
            </Button>
          </div>
        </div>
      )}
      ...
      {!isCreate && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("agent.config.identity")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("agent.config.identity")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field label={t("agent.config.name")} hint={t(help.name)}>
              <DraftInput
                value={eff("identity", "name", props.agent.name)}
                onCommit={(v) => mark("identity", "name", v)}
                immediate
                className={inputClass}
                placeholder={t("agent.config.name")}
              />
            </Field>
            <Field label={t("agent.config.title")} hint={t(help.title)}>
              <DraftInput
                value={eff("identity", "title", props.agent.title ?? "")}
                onCommit={(v) => mark("identity", "title", v || null)}
                immediate
                className={inputClass}
                placeholder={t("agent.config.titlePlaceholder")}
              />
            </Field>
            <Field label={t("agent.properties.reportsTo")} hint={t(help.reportsTo)}>
              <ReportsToPicker
                agents={companyAgents}
                value={eff("identity", "reportsTo", props.agent.reportsTo ?? null)}
                onChange={(id) => mark("identity", "reportsTo", id)}
                excludeAgentIds={[props.agent.id]}
                chooseLabel={t("agent.reports.chooseManager")}
              />
            </Field>
            <Field label={t("agent.config.capabilities")} hint={t(help.capabilities)}>
              <MarkdownEditor
                value={eff("identity", "capabilities", props.agent.capabilities ?? "") ?? ""}
                onChange={(v) => mark("identity", "capabilities", v || null)}
                placeholder={t("agent.config.capabilitiesPlaceholder")}
            ...
            {isLocal && !props.hidePromptTemplate && (
              <>
                <Field label={t("agent.config.promptTemplate")} hint={t(help.promptTemplate)}>
                  <MarkdownEditor
                    value={eff(
                      "adapterConfig",
                      "promptTemplate",
                    ...
                    placeholder={t("agent.config.promptTemplatePlaceholder")}
                ...
                <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                  {t("agent.config.promptTemplateWarning")}
                </div>
              </>
            )}
          </div>
      ...
      {environmentsEnabled ? (
        <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("agent.config.execution")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("agent.config.execution")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field
              label={t("agent.config.defaultEnvironment")}
              hint={t("agent.config.defaultEnvironmentHint")}
            >
              <select
                className={inputClass}
              ...
              >
                <option value="">{t("agent.config.companyDefaultLocal")}</option>
      ...
      <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
        <div className={cn(cards ? "flex items-center justify-between mb-3" : "px-4 py-2 flex items-center justify-between gap-2")}>
          {cards
            ? <h3 className="text-sm font-medium">{t("agent.config.adapter")}</h3>
            : <span className="text-xs font-medium text-muted-foreground">{t("agent.config.adapter")}</span>
          }
          {showInlineAdapterTestEnvironmentButton && (
            <Button
              ...
              disabled={testEnvironmentDisabled}
            >
              {testEnvironment.isPending ? t("agent.action.testing") : t("agent.config.test")}
            </Button>
          )}
        </div>
        <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
          {showAdapterTypeField && (
            <Field label={t("agent.config.adapterType")} hint={t(help.adapterType)}>
              <AdapterTypeDropdown
                value={adapterType}
          ...
          {showLegacyWorkingDirectoryField && (
            <Field label={t("agent.config.workingDirectoryDeprecated")} hint={t(help.cwd)}>
      ...
      {isLocal && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("agent.config.permissionsAndConfiguration")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("agent.config.permissionsAndConfiguration")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              <Field label={t("agent.config.command")} hint={t(help.localCommand)}>
                <DraftInput
                  value={
                    isCreate
                      ? val!.command
              ...
              {supportsModelProfiles && (
                <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{t("agent.config.primaryModel")}</div>
              )}
              <ModelDropdown
                models={models}
                ...
                detectModelLabel={t("agent.model.detect")}
                emptyDetectHint={t("agent.model.emptyDetect")}
              />
              ...
              {adapterType === "opencode_local"
                ...
                <p className="text-xs text-muted-foreground">
                  {t("agent.model.openCodeLocalOnly", { environment: currentDefaultEnvironment.name })}
                </p>
              )}
              ...
              {!isCreate && typeof config.bootstrapPromptTemplate === "string" && config.bootstrapPromptTemplate && (
                <>
                  <Field label={t("agent.config.bootstrapPrompt")} hint={t(help.bootstrapPrompt)}>
                    <MarkdownEditor
                      value={eff(
                        "adapterConfig",
                      ...
                      placeholder={t("agent.config.bootstrapPromptPlaceholder")}
                      contentClassName="min-h-[44px] text-sm font-mono"
                      imageUploadHandler={async (file) => {
                  ...
                  <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-200">
                    {t("agent.config.bootstrapPromptWarning")}
                  </div>
                </>
              )}
              ...
              <Field label={t("agent.config.extraArgs")} hint={t(help.extraArgs)}>
                <DraftInput
                  value={
                    isCreate
                      ? val!.extraArgs
                  ...
                  immediate
                  className={inputClass}
                  placeholder={t("agent.config.extraArgsPlaceholder")}
                />
              </Field>

              <Field label={t("agent.config.environmentVariables")} hint={t(help.envVars)}>
                <EnvVarEditor
                  value={
                    isCreate
              ...
              {!isCreate && (
                <>
                  <Field label={t("agent.config.timeout")} hint={t(help.timeoutSec)}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "timeoutSec",
                  ...
                  <Field label={t("agent.config.interruptGrace")} hint={t(help.graceSec)}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "graceSec",
      ...
      {isCreate && showCreateRunPolicySection ? (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium flex items-center gap-2 mb-3"><Heart className="h-3 w-3" /> {t("agent.config.runPolicy")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground flex items-center gap-2"><Heart className="h-3 w-3" /> {t("agent.config.runPolicy")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <ToggleWithNumber
              label={t("agent.config.heartbeatInterval")}
              hint={t(help.heartbeatInterval)}
              checked={val!.heartbeatEnabled}
              onCheckedChange={(v) => set!({ heartbeatEnabled: v })}
              number={val!.intervalSec}
              onNumberChange={(v) => set!({ intervalSec: v })}
              numberLabel={t("agent.config.runEverySuffix")}
              numberPrefix={t("agent.config.runEveryPrefix")}
        ...
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg overflow-hidden" : "")}>
            ...
            <CollapsibleSection
              title={t("agent.config.advancedRunPolicy")}
              bordered={cards}
            ...
            <div className="space-y-3">
              <ToggleField
                label={t("agent.config.wakeOnDemand")}
                hint={t(help.wakeOnDemand)}
                checked={eff(
                  "heartbeat",
                  "wakeOnDemand",
              ...
              />
              <Field label={t("agent.config.cooldown")} hint={t(help.cooldownSec)}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "cooldownSec",
              ...
              <Field label={t("agent.config.maxConcurrentRuns")} hint={t(help.maxConcurrentRuns)}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "maxConcurrentRuns",
              ...
              <div className="rounded-md border border-border/70 px-3 py-2">
                <ToggleField
                  label={t("agent.config.continueAfterMaxTurn")}
                  hint={t(help.maxTurnContinuationEnabled)}
                  checked={maxTurnContinuationEnabled}
                  onChange={(v) => updateMaxTurnContinuation({ enabled: v })}
                />
                {maxTurnContinuationEnabled ? (
                  <div className="mt-3 grid gap-3 sm:grid-cols-2">
                    <Field label={t("agent.config.continuationAttempts")} hint={t(help.maxTurnContinuationMaxAttempts)}>
                      <DraftNumberInput
                    ...
                    <Field label={t("agent.config.continuationDelay")} hint={t(help.maxTurnContinuationDelaySec)}>
                      <DraftNumberInput
...
export function AdapterEnvironmentResult({ result }: { result: AdapterEnvironmentTestResult }) {
  const { t } = useLanguage();
  const statusLabel =
    result.status === "pass" ? t("common.status.passed") : result.status === "warn" ? t("common.status.warnings") : t("common.status.failed");
  const statusClass =
    result.status === "pass"
      ...
      <div className="mt-2 space-y-1.5">
        {result.checks.map((check, idx) => (
          <div key={`${check.code}-${idx}`} className="text-[11px] leading-relaxed break-words">
            ...
            {check.hint && <span className="block opacity-90 break-words">{t("common.hint")}: {check.hint}</span>}
          </div>
        ))}
      </div>
    </div>
...
function ModelDropdown({
  ...
  return (
    <Field label={t("agent.config.model")} hint={t(help.model)}>
      <Popover
        open={open}
          ...
          <button type="button" className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>
              {selected
                ? selected.label
                : value
                  || (allowDefault ? (defaultLabel ?? t("agent.model.default")) : required ? t("agent.model.selectRequired") : t("agent.model.select"))}
            </span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
          <div className="relative mb-1">
            <input
              ...
              placeholder={creatable ? t("agent.model.searchCreate") : t("agent.model.search")}
              value={modelSearch}
          ...
          {onDetectModel && !modelSearch.trim() && (
            ...
            >
              ...
              {detectingModel ? t("agent.model.detecting") : detectedModel ? t("agent.model.redetect") : (detectModelLabel ?? t("agent.model.detectFromConfig"))}
            </button>
          )}
          {onRefreshModels && !modelSearch.trim() && (
            ...
            >
              ...
              {refreshingModels ? t("agent.model.refreshing") : t("agent.model.refresh")}
            </button>
          )}
          ...
          <div className="max-h-[240px] overflow-y-auto">
            ...
            {canCreateManualModel && (
              ...
              >
                <span>{t("agent.model.manual")}</span>
            ...
            {filteredModels.length === 0 && !canCreateManualModel && promotedModelIds.size === 0 && (
              <div className="px-2 py-2 space-y-2">
                <p className="text-xs text-muted-foreground">
                  {onDetectModel
                    ? (emptyDetectHint ?? t("agent.model.emptyDetectManual"))
                    : t("agent.model.empty")}
                </p>
              </div>
            )}
...
function CheapModelSection({
  ...
  const placeholderHint = adapterDefaultModel
    ...
    : t("agent.config.cheapModelPlaceholder");
  return (
    <div className="rounded-md border border-border/70 bg-muted/20 p-3 space-y-3">
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{t("agent.config.cheapModel")}</div>
          <p className="text-xs text-muted-foreground">
            {t("agent.config.cheapModelDescription")}
          </p>
        </div>
      ...
      {enabled && !model && adapterDefaultModel ? (
        <p className="text-[11px] text-muted-foreground">
          {t("agent.config.cheapModelUsesDefaultPrefix")} <code>{adapterDefaultModel}</code>.
        </p>
      ) : null}
      {enabled && !model && !adapterDefaultModel ? (
        <p className="text-[11px] text-amber-500">
          {t("agent.config.cheapModelNoDefault")}
        </p>
      ) : null}
    </div>
  );
...
function ThinkingEffortDropdown({
  ...
  const { t } = useLanguage();
  const optionLabel = (option: { id: string; label: string } | undefined) =>
    option ? t(`common.effort.${option.id || "auto"}`) : t("common.auto");

  return (
    <Field label={t("agent.config.thinkingEffort")} hint={t(help.thinkingEffort)}>
      <Popover open={open} onOpenChange={onOpenChange}>
        <PopoverTrigger asChild>
          <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>{optionLabel(selected)}</span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
